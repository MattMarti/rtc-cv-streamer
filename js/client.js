...
drone.on('open', error => {
    ...
    room.on('data', (message, sendingPeer) => {
        ...
        if (message.itIs) {
            let j = {wednesday : 'It is Wednesday My Dudes'};
            clientsMap.get(sendingPeer.id).sendMessage(j);
        }
    });
});
...
class RTC_Server_Connection {
    ...
    reactToMessage(message, sendingPeer) {
        if (message.sdp) {
            console.log(this.idStr + ' SDP answer received');
            let desc = new RTCSessionDescription(message.sdp);
            this.pc.setRemoteDescription(desc, () => {}, err => console.error(err));
        }
        if (message.candidate) { // Add new ICE candidate to remote description
            console.log(this.idStr + ' ICE candidate received');
